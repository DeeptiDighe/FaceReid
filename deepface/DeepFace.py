...
def represent_face(img, model_name = 'VGG-Face', model = None, enforce_detection = True, detector_backend = 'opencv', align = True, normalization = 'base'):
	...
	input_shape_x, input_shape_y = functions.find_input_shape(model)
	#import cv2
	#img = cv2.resize(img, dsize=(input_shape_x, input_shape_y))
	
	'''
	#detect and align
	img = functions.preprocess_face_image(img
		, target_size=(input_shape_y, input_shape_x)
		, enforce_detection = enforce_detection
		, detector_backend = detector_backend
		, align = align)
	'''
	img = functions.resize_face(img, (input_shape_y, input_shape_x))
	#---------------------------------
	#custom normalization
...
def detectFaceImage(img, target_size = (224, 224), detector_backend = 'opencv', enforce_detection = True, align = True):
	...
	img = functions.preprocess_face_image(img, target_size = target_size, detector_backend = detector_backend
		, enforce_detection = enforce_detection, align = align)[0] #preprocess_face returns (1, 224, 224, 3)
	return img[:, :, ::-1] #bgr to rgb
